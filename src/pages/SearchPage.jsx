...
function SearchPage(){
        ...
        switch (query){
            case "0 - 5.000 kg":
                return dinosaursData.filter(item=>
                    item.weight >0 && item.weight <=5000);
            case "5.000 - 10.000 kg":
                return dinosaursData.filter(item=>
                    item.weight >5000 && item.weight <=10000);
            case "10.000 - 15.000 kg":
                return dinosaursData.filter(item=>
                    item.weight >10000 && item.weight <=15000);
            case "> 15.000 kg":
                return dinosaursData.filter(item=>
                    item.weight >15000);
            case "< 1m":
            ...
            case "1 - 20m":
                return dinosaursData.filter(item=>
                    item.length >1 && item.length <=20);
            case "20 - 40m":
                return dinosaursData.filter(item=>
                    item.length >20 && item.length <=40);
            case "> 40m":
                return dinosaursData.filter(item=>
                    item.length >40);
            default: 
    ...
    return (
       ...
       htmlFor="searchBar">Search by name, country and diet</label>
        <input 
            value= {query}
      ...
      <br></br>
       ...
       <div>
            ...
            <label htmlFor="weight">Filter by weight:</label>
                <select name="weight" id="weight" onChange={(e)=> setQuery(e.target.value)}>
                    <option value=""></option>
            ...
            <label htmlFor="length">Filter by length:</label>
                <select name="length" id="length" onChange={(e)=> setQuery(e.target.value)}>
                    <option value=""></option>
